...
class TabuCustom:
    ...
    def find_best_SwapTwoWithOneMove(self, stwom):  # swaps in both max route and other routes without increasing
        # max_cost
                ...
                for secondNodeIndex in range(startOfSecondNodeIndex, len(
                    ...
                    if max_route_index == route2_index:  # if the routes are same
                        if firstNodeIndex == secondNodeIndex - 1:  # if the first node is behind the second node
                            ...
                            costAdded = self.time_matrix[a1.ID][b2.ID] + self.time_matrix[c2.ID][b1.ID] + \
                                        self.time_matrix[b1.ID][d2.ID]
                            moveCost = costAdded - costRemoved

                        else:
                            costRemoved1 = self.time_matrix[a1.ID][b1.ID] + self.time_matrix[b1.ID][c1.ID]
                            costAdded1 = self.time_matrix[a1.ID][b2.ID] + self.time_matrix[c2.ID][c1.ID]
                            costRemoved2 = self.time_matrix[a2.ID][b2.ID] + self.time_matrix[c2.ID][d2.ID]
                            costAdded2 = self.time_matrix[a2.ID][b1.ID] + self.time_matrix[b1.ID][d2.ID]
                            moveCost = costAdded1 + costAdded2 - (costRemoved1 + costRemoved2)
                    else:
                        if max_route.load - b1.demand + b2.demand + c2.demand> self.capacity:
                            continue
                        if route2.load - b2.demand -c2.demand + b1.demand > self.capacity:
                            continue

                        costRemoved1 = self.time_matrix[a1.ID][b1.ID] + self.time_matrix[b1.ID][c1.ID]
                        costAdded1 = self.time_matrix[a1.ID][b2.ID] + self.time_matrix[c2.ID][c1.ID]+ self.time_matrix[b2.ID][c2.ID]
                        costRemoved2 = self.time_matrix[a2.ID][b2.ID] + self.time_matrix[c2.ID][d2.ID]+ self.time_matrix[b2.ID][c2.ID]
                        costAdded2 = self.time_matrix[a2.ID][b1.ID] + self.time_matrix[b1.ID][d2.ID]
                    ...
                    if (moveCost < stwom.moveCost or
                        ...
                        if moveCost == stwom.moveCost:
                            ...
                            moves_with_same_cost = 2
                        self.StoreBestSwapMove(route2_index, max_route_index, secondNodeIndex, firstNodeIndex,
                                               moveCost, costChangeSecondRoute, costChangeFirstRoute, stwom)
                ...
                for secondNodeIndex in range(startOfSecondNodeIndex, len(
                    ...
                    if max_route_index == route2_index:  # if the routes are same
                       ...
                       # print("case3")
                        ...
                        costRemoved1 = self.time_matrix[a1.ID][b1.ID] + self.time_matrix[c1.ID][d1.ID]+self.time_matrix[b1.ID][c1.ID]
                        costAdded1 = self.time_matrix[a1.ID][b2.ID] + self.time_matrix[b2.ID][d1.ID]
                        costRemoved2 = self.time_matrix[a2.ID][b2.ID] + self.time_matrix[b2.ID][c2.ID]
                        costAdded2 = self.time_matrix[a2.ID][b1.ID] + self.time_matrix[c1.ID][c2.ID]+self.time_matrix[b1.ID][c1.ID]
    ...
    def FindBestSwapTwoWithOneMove2(self, stwom):  # antono-sider 2 nodes leave from route 1 and 1 gets there from route 2 
        unpack = self.FindRouteWithMaxCost()  # find the Route with the max cost and its index in the routes matrix
        max_route = unpack[1]  # unpack Route
        moves_with_same_cost = 2
        for route1_index in range(0, len(self.sol.routes)):
                    ...
                    if route2 != max_route:
                        for firstNodeIndex in range(1,
                            ...
                            for secondNodeIndex in range(startOfSecondNodeIndex, len(
                                ...
                                if self.MoveIsTabuForSwapTwoWithOneMove(a1.ID, b1.ID, c1.ID,d1.ID, a2.ID, b2.ID, c2.ID):
                                    continue
    ...
    def ApplySwapTwoWithOneMove(self, stwom):
        print(stwom.positionOfFirstRoute, )
        ...
        print("b2:",b2.ID)
        print(rt1.sequenceOfNodes)
        rt1.sequenceOfNodes[stwom.positionOfFirstNode : stwom.positionOfFirstNode + 2] = [b2]
    ...
    def StoreBestSwapTwoWithOneMoveMove(self, firstRouteIndex, secondRouteIndex, firstNodeIndex, secondNodeIndex, moveCost,
                          costChangeFirstRoute, costChangeSecondRoute, sm):
        sm.positionOfFirstRoute = firstRouteIndex
        sm.positionOfSecondRoute = secondRouteIndex
        sm.positionOfFirstNode = firstNodeIndex
