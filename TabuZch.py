...
class TabuCustom:
    ...
    def find_best_relocation_move_max_and_other(self, rm,
                                                localSearchIterator):  # relocations in both max route and other routes without
        # increasing max_cost
    ...
    def find_best_relocation_for_max_route_and_another_route(self, relocation_move, max_route_index, route2_index,
        ...
        for originNodeIndex in range(1, len(
            ...
            for targetNodeIndex in range(0, len(
                ...
                #     continue
                if (self.MoveIsTabu(B, localSearchIterator, move_cost, True)):
                    continue
    ...
    def find_best_swap_move_max_and_other(self, sm,
                                          localSearchIterator):  # swaps in both max route and other routes without increasing
        # max_cost
        ...
        for route2_index in range(0, len(
            ...
            for firstNodeIndex in range(1, len(max_route.sequenceOfNodes) - 1):  # for every node of the first route
                ...
                for secondNodeIndex in range(startOfSecondNodeIndex, len(
                    ...
                    if self.MoveIsTabu(b1, localSearchIterator, moveCost, True) or self.MoveIsTabu(b2,
                                                                                                   localSearchIterator,
                                                                                                   moveCost, True):
                        continue
                    ...
                    if route2 != max_route:
                        for firstNodeIndex in range(1,
                            ...
                            for secondNodeIndex in range(startOfSecondNodeIndex, len(
                                ...
                                if self.MoveIsTabu(b1, localSearchIterator, moveCost) or self.MoveIsTabu(b2,
                                                                                                         localSearchIterator,
                                                                                                         moveCost):
                                    continue
    ...
    def FindBestTwoOptMove_MaxRoute(self, top, rtInd1,
                                    rtInd2,
                                    localSearchIterator):  # spy ---> this method finds the best 2-opt move, which is the one that reduces cost the most (needs current best as input)
        ...
        for nodeInd1 in range(0, len(
            ...
            for nodeInd2 in range(start2, len(rt2.sequenceOfNodes) - 1):
                ...
                if self.MoveIsTabu(A, localSearchIterator, moveCost, True) or self.MoveIsTabu(K, localSearchIterator,
                                                                                              moveCost, True):
                    continue
    ...
    def FindBestTwoOptMove_notMaxRoute(self, top, rtInd1,
                                       rtInd2,
                                       localSearchIterator):  # spy + mark ---> this method finds the best 2-opt move, which is the one that reduces cost the most (needs current best as input)
        rt1: Route = self.sol.routes[rtInd1]
        rt2: Route = self.sol.routes[rtInd2]
        route_max = self.CalculateMaxCostOfRoute()
        route1_current_cost = 0  # the current cost of the route based on the loop (0,A,B------
    ...
    def MoveIsTabuForSwaps(self, a1ID, b1ID, c1ID, a2ID, b2ID, c2ID, iterator, moveCost, route_is_max=False):
        if route_is_max and moveCost + self.sol.max_cost_of_route < self.bestSolution.max_cost_of_route - 0.001:
            return False
            # iterator = 30
        if (self.TabuForbiddenArcs[a1ID][b2ID] > iterator) or (self.TabuForbiddenArcs[b2ID][c1ID] > iterator) or (
                self.TabuForbiddenArcs[a2ID][b1ID] > iterator) or (self.TabuForbiddenArcs[b1ID][c2ID] > iterator):
